...
exports.handler = async function (context, event, callback) {
  ...
  const createProxySession = async (chatChannelSid) => {
    ...
    try {
      ...
      if (proxySession) {
        console.debug(`Found EXISTING Flex Proxy Session between Chats Channel SID '${chatChannelSid}' and toNumber '${toNumber}'`);
        // response.setStatusCode(409);
        // response.setBody({ message: 'Found already existing Flow', proxySession });
